...
public class board {
	...
	public void move(int col1, int row1, int col2, int row2) {
		...
		if (whiteKing.check(board) || blackKing.check(board)) {
		...
		if (blackPieces == 1 || whitePieces == 1) {
			...
			if (blackPieces == 1) {
				stalemate = staleMate(blackKing);
			} else if (whitePieces == 1) {
				stalemate = staleMate(whiteKing);
	...
	public boolean checkMate(piece king) {
		...
		if (checkCount == 1 && freeSpaces == 0) {
			...
			if (Math.abs(checker.column - column) == 1 && Math.abs(checker.rows - rows) == 1) {
				...
				piece temp = new king(checker.column, checker.rows, king.color, king.name, king.id);

				boolean check = temp.check(board);
	...
	public boolean staleMate(piece king) {
		...
		if (column + 1 != 9) {
			...
			piece temp = new king(column + 1, rows, king.color, king.name, "king");
		...
		if (rows + 1 != 9) {
			...
			piece temp = new king(column, rows + 1, king.color, king.name, "king");
		...
		if (column + 1 != 9 && rows + 1 != 9) {
			...
			piece temp = new king(column + 1, rows + 1, king.color, king.name, "king");
